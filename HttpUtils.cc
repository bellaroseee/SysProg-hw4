...
bool IsPathSafe(const string &rootdir, const string &testfile) {
  ...
  // STEP 1
  char rootdir_path[PATH_MAX];
  char testfile_path[PATH_MAX];
  if (!realpath(rootdir.c_str(), rootdir_path)) {
    return false;
  }

  if (!realpath(testfile.c_str(), testfile_path)) {
    return false;
  }

  if (strlen(testfile_path) <= strlen(rootdir_path)) {
    return false;
  }

  if (strncmp(rootdir_path, testfile_path, strlen(rootdir_path)) != 0) {
    return false;
  }
